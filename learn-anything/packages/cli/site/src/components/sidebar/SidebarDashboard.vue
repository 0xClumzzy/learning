...
<template>
  <nav class="flex-1 overflow-y-auto px-6 py-4">
    <p
      ...
      class="text-xs font-semibold text-text-3 uppercase tracking-wider mb-2"
    >
      Topics
    </p>
    ...
    <div class="space-y-0.5">
      <button
        ...
        :key="topic.slug"
        class="w-full text-left px-3 py-1.5 rounded-[10px] text-sm font-medium text-text-2 hover:text-text-1 hover:bg-white/5 transition-all cursor-pointer"
    ...
    <div v-if="topics.length === 0" class="text-xs text-text-3 mt-2">
